...
<head>
    ...
    <title>Kegiatan Masjid | Sistem Informasi Manajemen Masjid</title>
        ...
        <div class="max-w-8xl mx-auto px-8 sm:px-14 lg:px-28">
            <div class="max-w-3xl" data-aos="fade-up">
                ...
                <p class="mt-5 text-lg text-emerald-100 leading-8">
                    Lihat seluruh agenda kegiatan yang sedang dan akan diselenggarakan oleh masjid.
                </p>
            </div>
        </div>
    </section>
            ...
            <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-8">
                @forelse($kegiatan ?? [] as $index => $item)
                    <div class="bg-white rounded-3xl shadow-lg overflow-hidden hover:-translate-y-2 transition duration-300" data-aos="fade-up" @if($index > 0) data-aos-delay="{{ $index * 100 }}" @endif>
                        <img src="{{ $item->gambar ? asset('storage/'.$item->gambar) : asset('assets/images/no-image.png') }}" class="w-full h-60 object-cover">
                        <div class="p-7">
                ...
                @empty
                    <div class="col-span-full text-center py-20">
                        ...
                        <h3 class="mt-5 text-2xl font-semibold">Belum ada kegiatan</h3>
                        <p class="text-slate-500 mt-2">Kegiatan yang aktif akan tampil di sini.</p>
                    </div>
                @endforelse
            </div>

            @if(($kegiatan ?? collect())->hasPages())
